...
- Keep the pipeline in [architecture](.agents/architecture.md): direct ANTLR YQL
...
- Keep runtime SDK dependencies in tests/examples, out of the generator's module
  ...
  pinned dependencies and record API evidence in
  [SDK evidence](.agents/sdk-evidence.md). Keep source attribution in
...
- Run the affected package tests while editing; run `make check` for the integrated
  change. Commands and prerequisites are in [development](.agents/development.md).
- When output changes intentionally, regenerate examples with `make generate` and
  update [golden fixtures](.agents/development.md#golden-fixtures). Review the generated diff;
...
- User documentation is planned for ydb.tech near release. Keep installation,
  compatibility, generated API and runtime ownership contracts in `docs/`; once
  the site guide exists, link it rather than maintaining two full user journeys.
  Contributor commands and implementation playbooks belong in
  [development](.agents/development.md). Update the canonical page when behavior
  changes, then link to it elsewhere.
- Release preparation is tracked in [the release plan](.agents/release-plan.md).
...
- `.agents/` contains maintained project memory and maintainer-only technical
  documentation: the code map, lasting decisions, implementation boundaries,
  contributor and release playbooks, source/API evidence and dated readiness
  snapshots. Update it when those facts change. Keep generated APIs and consumer
  contracts in public documentation. Label plans as planned and point-in-time CI
  or dependency observations with their date and evidence; tests and source
