...
func TestLiveYDBSharedExpressions(t *testing.T) {
	...
	for _, runtime := range []string{"ydb", "database/sql"} {
		t.Run(runtime, func(t *testing.T) {
			...
			source := strings.NewReplacer("$DSN", strconv.Quote(dsn), "$SCHEMA", strconv.Quote(schema), "$TABLE", strconv.Quote(table), "$SETUP", setup, "$SQL_IMPORT", sqlImport, "$METADATA", metadata.String()).Replace(sharedExpressionsRuntime)
