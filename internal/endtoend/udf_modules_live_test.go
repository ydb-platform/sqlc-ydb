...
func TestLiveYDBUDFModules(t *testing.T) {
	cases := []struct {
		...
		{"text_collections", `SELECT Yson::SerializeJson(Yson::From(String::SplitToList("a,b", ","))) AS words,
			...
			map[string]any{"words": `["a","b"]`, "unicode_words": `["a","b"]`, "query_pairs": `[["a","1"],["b","2"]]`}},
		...
		{"documents", `SELECT Yson::IsString(Yson::From("book")) AS is_string,
			...
			map[string]any{"is_string": true, "name": "book", "numbers": `["a","b"]`, "converted": 42, "length": 2}},
		...
		{"bitmaps", `SELECT Roaring::Cardinality(Roaring::FromUint32List(Yson::ConvertTo(Yson::ParseJson(CAST("[1,2,2]" AS Json)), List<Uint32>))) AS cardinality,
		    Roaring::Cardinality(Roaring::And(Roaring::FromUint32List(Yson::ConvertTo(Yson::ParseJson(CAST("[1,2]" AS Json)), List<Uint32>)), Roaring::FromUint32List(Yson::ConvertTo(Yson::ParseJson(CAST("[2,3]" AS Json)), List<Uint32>)))) AS intersection;`,
