...
func TestLiveYDBSemanticTypes(t *testing.T) {
	...
	queries := []struct{ Name, SQL string }{
		...
		{"group_computed", "SELECT doubled, COUNT(*) AS total FROM $TABLE GROUP BY n + n AS doubled HAVING doubled > 0;"},
		{"group_computed_where", "SELECT doubled, COUNT(*) AS total FROM $TABLE WHERE doubled > 0 GROUP BY n + n AS doubled;"},
		{"group_alias_shadow", "SELECT n, COUNT(*) AS total FROM $TABLE WHERE n > 0ul GROUP BY id AS n;"},
