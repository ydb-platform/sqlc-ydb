...
func generateDatabaseSQL(ctx context.Context, req *pb.GenerateRequest) (*pb.GenerateResponse, error) {
	...
	models, err := dbsqlGenModels(req.GetCatalog(), req.GetQueries(), pkg)
	if err != nil {
		return nil, err
	}
	files = append(files, &pb.File{Name: "models.go", Contents: models})

	db, err := dbsqlGenDB(pkg)
	if err != nil {
		return nil, err
	}
	...
	for fname, qs := range byFilename {
		...
		content, err := dbsqlGenQueryFile(req.GetCatalog(), pkg, fname, qs)
		if err != nil {
			return nil, err
		}
...
func dbsqlGenModels(cat *pb.Catalog, queries []*pb.Query, pkg string) ([]byte, error) {
...
func dbsqlGenDB(pkg string) ([]byte, error) {
...
func dbsqlGenQueryFile(cat *pb.Catalog, pkg, sourceName string, queries []*pb.Query) ([]byte, error) {
	var buf bytes.Buffer
	buf.WriteString("// Code generated by sqlc. DO NOT EDIT.\n// source: " + sourceName + "\n\n")
	buf.WriteString("package " + pkg + "\n\n")
	...
	for i, q := range queries {
		block, err := dbsqlGenQuery(cat, pkg, q)
		if err != nil {
			return nil, err
		}
		buf.Write(block)
...
func dbsqlGenQuery(cat *pb.Catalog, pkg string, q *pb.Query) ([]byte, error) {
	var buf bytes.Buffer
	name := q.GetName()
	cmd := strings.ToLower(q.GetCmd())
	text := q.GetText()
	...
	switch cmd {
	case ":one":
		buf.WriteString(dbsqlGenOne(cat, name, constName, q))
	case ":many":
		buf.WriteString(dbsqlGenMany(cat, name, constName, q))
	default:
		buf.WriteString(dbsqlGenExec(name, constName, q))
	}
	return buf.Bytes(), nil
}

func dbsqlNamedArgList(params []*pb.Parameter) string {
	if len(params) == 0 {
		return ""
	}
	var parts []string
...
func dbsqlGenOne(cat *pb.Catalog, name, constName string, q *pb.Query) string {
	var b bytes.Buffer
	params := q.GetParams()
	cols := q.GetColumns()
	retType := "interface{}"
...
func dbsqlGenMany(cat *pb.Catalog, name, constName string, q *pb.Query) string {
	var b bytes.Buffer
	params := q.GetParams()
	cols := q.GetColumns()
	rowType := "interface{}"
...
func dbsqlGenExec(name, constName string, q *pb.Query) string {
	var b bytes.Buffer
	params := q.GetParams()
	sig := "ctx context.Context"
	args := ""
