			...
			for _, file := range files {
				...
				for _, want := range tc.want {
					if !strings.Contains(code, want) {
						t.Fatalf("generated query is missing %q:\n%s", want, code)
					}
				}
				if strings.Contains(code, "WHERE id = ?") {
					t.Fatalf("named parameter became a positional placeholder:\n%s", code)
				}
				return
			}
...
func TestJooqTabularNamedSQLCompilesAndPreservesText(t *testing.T) {
	...
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "Main.java"), []byte(program), 0600); err != nil {
		t.Fatal(err)
	}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if got := jdbc.NamedSQL(analysis.Queries[0]); got != wantSQL {
				t.Fatalf("named SQL = %q, want %q", got, wantSQL)
			}
			...
			for _, file := range files {
				...
				if strings.Contains(code, "id = ?") || strings.Contains(code, "_prepared.setObject(\"id\", null)") {
					t.Fatalf("named parameter was lost:\n%s", code)
				}
				return
			}
			t.Fatal("Queries.java was not generated")
