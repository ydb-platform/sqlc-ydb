...
func Generate(in *model.AnalysisResult, o Options) ([]model.File, error) {
	...
	if needsDecimalValidation {
		imports := `"fmt"; "github.com/ydb-platform/ydb-go-sdk/v3/types"`
		failure := `fmt.Errorf("Decimal parameter %s expects Decimal(%d,%d), got Decimal(%d,%d)", name, precision, scale, value.Precision, value.Scale)`
		beforeReturn := "\n"
		if o.Runtime == "ydb" {
			imports = "\"fmt\"\n\n" + `"github.com/ydb-platform/ydb-go-sdk/v3/pkg/xerrors"; "github.com/ydb-platform/ydb-go-sdk/v3/types"`
			failure = "xerrors.WithStackTrace(" + failure + ")"
			beforeReturn = "\n\n"
		}
...
func queryFile(source string, qs []model.AnalyzedQuery, o Options) []byte {
	...
	needsJSON, needsSQLNamed, needsTypes, needsUUID, needsYDB, needsYDBRows := false, false, false, false, false, false
	needsEach := false
	...
	if o.Runtime == "database/sql" {
		...
		if needsYDBRows {
			stdlibImports = append(stdlibImports, "\"io\"")
		}
		if needsYDB {
		...
		externalImports = append(externalImports, "\"github.com/ydb-platform/ydb-go-sdk/v3/pkg/xerrors\"")
		if needsTypes {
			externalImports = append(externalImports, "\"github.com/ydb-platform/ydb-go-sdk/v3/types\"")
		}
		if needsUUID {
...
func queryAnnotation(line string) bool {
	...
	return strings.HasPrefix(line, "--") && strings.Contains(line, "name:")
}

func queryComment(q model.AnalyzedQuery) string {
