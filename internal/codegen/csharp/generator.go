...
func validate(in *model.AnalysisResult) error {
	...
	for _, n := range []string{"Guid", "Task", "CancellationToken", "List", "IReadOnlyList", "DbType", "DBNull", "YdbConnection", "YdbTransaction", "YdbCommand", "YdbParameter"} {
...
func writeParameter(b *bytes.Buffer, q model.AnalyzedQuery, p model.Parameter) {
	v := parameterRef(q, p)
	fmt.Fprintf(b, "        command.Parameters.Add(new YdbParameter(%q, DbType.%s, %s));\n", "$"+p.Name, dbType(p.Type), nullableValue(p.Type, v))
}
func nullableValue(t model.Type, v string) string {
	if t.IsOptional() {
