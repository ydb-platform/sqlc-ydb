...
func Generate(in *model.AnalysisResult, o Options) ([]model.File, error) {
	...
	if len(in.Diagnostics) != 0 {
		return nil, fmt.Errorf("csharp generator: cannot generate with diagnostics: %w", in.Diagnostics[0])
	}
	if o.Namespace == "" {
		o.Namespace = "Db"
	}
...
func validate(in *model.AnalysisResult, runtime string) error {
	...
	for _, q := range in.Queries {
		...
		if q.Command == model.One || q.Command == model.Many {
			...
			columns, _ := csRowColumns(result)
			if err := fieldNames("query "+q.Name, csName(q.Name)+"Row", columns); err != nil {
				return err
			}
			for _, column := range result.Columns {
				if _, err := csType(column.Type); err != nil {
					return fmt.Errorf("csharp generator: query %q result column %q: %w", q.Name, column.Name, err)
				}
			}
			for _, embed := range result.Embeds {
				if embed.Start < 0 || embed.End > len(result.Columns) || embed.Start >= embed.End {
					return fmt.Errorf("csharp generator: query %q: invalid embedded column range", q.Name)
				}
				...
				if table == nil || len(table.Columns) != embed.End-embed.Start {
					return fmt.Errorf("csharp generator: query %q: embedded table %q does not match projected columns", q.Name, embed.Table)
				}
			...
			if runtime == "dapper" && len(result.Embeds) > 0 {
				if err := add(modelNames, csName(q.Name)+"WireRow", "wire row:"+q.Name, "model name"); err != nil {
					return err
				}
			}
		}
...
func renderModels(in *model.AnalysisResult, o Options) []byte {
	...
	for _, q := range in.Queries {
		...
		if q.Command == model.One || q.Command == model.Many {
			columns, overrides := csRowColumns(q.ResultSets[0])
			write(csName(q.Name)+"Row", columns, overrides)
		}
	}
	return []byte(strings.TrimRight(b.String(), "\n") + "\n")
}

func csRowColumns(result model.ResultSet) ([]model.Column, map[string]string) {
	...
	for i := 0; i < len(result.Columns); i++ {
		if embed := csEmbeddingAt(result.Embeds, i); embed != nil {
			columns = append(columns, model.Column{Name: embed.Field})
			overrides[embed.Field] = csName(embed.Table)
			i = embed.End - 1
			continue
		}
...
func csEmbeddingAt(embeds []model.Embedding, index int) *model.Embedding {
	for i := range embeds {
...
// Both profiles borrow the connection and transaction from the caller.
const connectionMembers = `    private readonly YdbConnection _connection;
...
func writeDapperWireRows(b *bytes.Buffer, in *model.AnalysisResult) {
	for _, q := range in.Queries {
		if len(q.ResultSets) == 0 || len(q.ResultSets[0].Embeds) == 0 {
			continue
		}
		fmt.Fprintf(b, "\n    private sealed record %sWireRow(\n", csName(q.Name))
		columns := q.ResultSets[0].Columns
		for i, column := range columns {
			typ, _ := csType(column.Type)
			comma := ","
			if i == len(columns)-1 {
				comma = ""
			}
...
func writeDapperMaps(b *bytes.Buffer, in *model.AnalysisResult) {
	...
	for _, q := range mapped {
		rowType := csName(q.Name) + "Row"
		if len(q.ResultSets[0].Embeds) > 0 {
			rowType = csName(q.Name) + "WireRow"
		}
		fmt.Fprintf(b, "        SqlMapper.SetTypeMap(typeof(%s), new ColumnTypeMap(typeof(%s), new Dictionary<string, string>\n        {\n", rowType, rowType)
		for i, c := range q.ResultSets[0].Columns {
			if len(q.ResultSets[0].Embeds) > 0 {
				fmt.Fprintf(b, "            [%s] = nameof(%s.Column%d),\n", csString(c.ResultName()), rowType, i)
			} else if !strings.EqualFold(c.ResultName(), csName(c.Name)) {
				fmt.Fprintf(b, "            [%s] = nameof(%s.%s),\n", csString(c.ResultName()), rowType, csName(c.Name))
			}
		}
		b.WriteString("        }));\n")
	}
...
func writeDapperMethod(b *bytes.Buffer, q model.AnalyzedQuery) {
	...
	case model.One:
		if len(q.ResultSets[0].Embeds) > 0 {
			fmt.Fprintf(b, "        var row = await _connection.QueryFirstAsync<%sWireRow>(command).ConfigureAwait(false);\n        return %sRowFromWire(row);\n", name, name)
		} else {
			fmt.Fprintf(b, "        return await _connection.QueryFirstAsync<%sRow>(command).ConfigureAwait(false);\n", name)
		}
	case model.Many:
		if len(q.ResultSets[0].Embeds) > 0 {
			fmt.Fprintf(b, "        return (await _connection.QueryAsync<%sWireRow>(command).ConfigureAwait(false)).Select(%sRowFromWire).ToList();\n", name, name)
		} else {
			fmt.Fprintf(b, "        return (await _connection.QueryAsync<%sRow>(command).ConfigureAwait(false)).AsList();\n", name)
		}
	}
	b.WriteString("    }\n")
	if (q.Command == model.One || q.Command == model.Many) && len(q.ResultSets[0].Embeds) > 0 {
		writeDapperEmbeddedMapper(b, q)
	}
}

func writeDapperEmbeddedMapper(b *bytes.Buffer, q model.AnalyzedQuery) {
	name := csName(q.Name)
	fmt.Fprintf(b, "\n    private static %sRow %sRowFromWire(%sWireRow row) => new(\n", name, name, name)
	result := q.ResultSets[0]
	for i := 0; i < len(result.Columns); i++ {
		if embed := csEmbeddingAt(result.Embeds, i); embed != nil {
			fmt.Fprintf(b, "        new %s(\n", csName(embed.Table))
			for j := embed.Start; j < embed.End; j++ {
				...
				fmt.Fprintf(b, "            row.Column%d%s\n", j, comma)
			}
			comma := ","
			if embed.End == len(result.Columns) {
				comma = ""
			}
			fmt.Fprintf(b, "        )%s\n", comma)
			i = embed.End - 1
	...
	for i := 0; i < len(result.Columns); i++ {
		if embed := csEmbeddingAt(result.Embeds, i); embed != nil {
			...
			for j := embed.Start; j < embed.End; j++ {
				...
				fmt.Fprintf(b, "            %s%s\n", readValue(result.Columns[j], j), comma)
			}
			comma := ","
			if embed.End == len(result.Columns) {
				comma = ""
		...
		fmt.Fprintf(b, "        %s%s\n", readValue(result.Columns[i], i), comma)
	}
	b.WriteString("    );\n")
}
