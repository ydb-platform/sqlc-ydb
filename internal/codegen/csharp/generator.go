...
func writeLinq2DBMethod(b *bytes.Buffer, q model.AnalyzedQuery) {
	...
	if q.Command == model.One {
		fmt.Fprintf(b, "        await foreach (var row in _connection.QueryToAsyncEnumerable(%sRowFrom,%s%s).WithCancellation(cancellationToken).ConfigureAwait(false))\n        {\n            return row;\n        }\n        throw new InvalidOperationException(\"query returned no rows\");\n", name, sqlLiteral(model.WithoutQueryAnnotation(q.SQL)), params)
	} else {
		fmt.Fprintf(b, "        return await _connection.QueryToListAsync(%sRowFrom,%s, cancellationToken%s).ConfigureAwait(false);\n", name, sqlLiteral(model.WithoutQueryAnnotation(q.SQL)), params)
	}
	b.WriteString("    }\n")
	writeRowMapper(b, q)
}
