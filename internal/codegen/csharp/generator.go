...
func Generate(in *model.AnalysisResult, o Options) ([]model.File, error) {
	...
	default:
		return nil, fmt.Errorf("csharp generator: unsupported runtime %q (supported: adonet, dapper, linq2db)", o.Runtime)
	}
...
func validate(in *model.AnalysisResult, runtime string) error {
	...
	for _, n := range []string{"Guid", "DateTime", "Task", "CancellationToken", "List", "IReadOnlyList", "DbType", "DBNull", "DbDataReader", "ArgumentNullException", "InvalidOperationException", "YdbConnection", "YdbTransaction", "YdbCommand", "YdbParameter", "YdbValue"} {
		modelNames[n] = "framework type"
	}
	if runtime == "dapper" {
		for _, n := range []string{"CommandDefinition", "SqlMapper"} {
			modelNames[n] = "framework type"
		}
	}
	if runtime == "linq2db" {
		for _, n := range []string{"DataConnection", "DataParameter", "DataType"} {
			modelNames[n] = "framework type"
		}
	}
...
func renderQueries(in *model.AnalysisResult, o Options) []byte {
	...
	case "linq2db":
		return renderLinq2DBQueries(in, o)
	}
	var b bytes.Buffer
...
func writeDapperMethod(b *bytes.Buffer, q model.AnalyzedQuery) {
	...
	fmt.Fprintf(b, "        var command = new CommandDefinition(Sql%s, %s, _transaction, cancellationToken: cancellationToken);\n", name, dapperParameters(q))
	if q.Command == model.Exec {
		b.WriteString("        await _connection.ExecuteAsync(command).ConfigureAwait(false);\n")
		b.WriteString("    }\n")
		return
	}
	b.WriteString("        await using var reader = await _connection.ExecuteReaderAsync(command).ConfigureAwait(false);\n")
	if q.Command == model.One {
		b.WriteString("        if (!await reader.ReadAsync(cancellationToken).ConfigureAwait(false))\n        {\n            throw new InvalidOperationException(\"query returned no rows\");\n        }\n        return " + name + "RowFrom(reader);\n")
	} else {
		b.WriteString("        var rows = new List<" + name + "Row>();\n        while (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))\n        {\n            rows.Add(" + name + "RowFrom(reader));\n        }\n        return rows;\n")
	}
	b.WriteString("    }\n")
	writeRowMapper(b, q)
}
...
func renderLinq2DBQueries(in *model.AnalysisResult, o Options) []byte {
	var b bytes.Buffer
	b.WriteString("// Code generated by sqlc-ydb. DO NOT EDIT.\n#nullable enable\nusing System;\nusing System.Collections.Generic;\nusing System.Data.Common;\nusing System.Threading;\nusing System.Threading.Tasks;\nusing LinqToDB;\nusing LinqToDB.Data;\nusing Ydb.Sdk.Value;\n\nnamespace " + o.Namespace + ";\n\n")
	b.WriteString("public sealed class Queries\n{\n    private readonly DataConnection _connection;\n\n    public Queries(DataConnection connection)\n    {\n        _connection = connection ?? throw new ArgumentNullException(nameof(connection));\n    }\n")
	for _, q := range in.Queries {
		writeSQLConstant(&b, q)
		writeLinq2DBMethod(&b, q)
	}
	b.WriteString("}\n")
	return b.Bytes()
}

func writeLinq2DBMethod(b *bytes.Buffer, q model.AnalyzedQuery) {
	name := csName(q.Name)
	ret := "Task"
	if q.Command == model.One {
		ret = "Task<" + name + "Row>"
	...
	if q.Command == model.Exec {
		fmt.Fprintf(b, "        await _connection.ExecuteAsync(Sql%s, cancellationToken%s).ConfigureAwait(false);\n", name, params)
		b.WriteString("    }\n")
		return
	}
	fmt.Fprintf(b, "        var rows = await _connection.QueryToListAsync(%sRowFrom, Sql%s, cancellationToken%s).ConfigureAwait(false);\n", name, name, params)
	if q.Command == model.One {
...
func linq2DBParameters(q model.AnalyzedQuery) string {
	if len(q.Parameters) == 0 {
		return ""
	}
	var b strings.Builder
	for _, p := range q.Parameters {
		b.WriteString(", ")
		b.WriteString(linq2DBParameterExpression(q, p))
	}
	return b.String()
}

func linq2DBParameterExpression(q model.AnalyzedQuery, p model.Parameter) string {
	v := parameterRef(q, p)
	t := p.Type.UnwrapOptional()
	value := v
	if p.Type.IsOptional() {
		value = "YdbValue.MakeOptional" + csName(t.Kind) + "(" + v + ")"
	} else if strings.EqualFold(t.Kind, "json") || strings.EqualFold(t.Kind, "timestamp") {
		value = "YdbValue.Make" + csName(t.Kind) + "(" + v + ")"
	}
	return fmt.Sprintf("new DataParameter(%q, %s, DataType.%s)", "$"+p.Name, value, linq2DBDataType(t))
}

func linq2DBDataType(t model.Type) string {
	switch strings.ToLower(t.Kind) {
	case "bool":
		return "Boolean"
	case "float":
