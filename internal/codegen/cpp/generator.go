...
func validate(in *model.AnalysisResult, options Options) error {
	...
	for _, query := range in.Queries {
		if err := validateIdent(query.Name); err != nil || query.Name == "Queries" || query.Name == "client_" || query.Name == "transaction_" {
...
func renderHeader(in *model.AnalysisResult, options Options) string {
	...
	out.WriteString("\nnamespace " + options.Namespace + " {\n\nclass Queries final {\npublic:\n")
	out.WriteString("    explicit Queries(" + clientType + "& client) noexcept : client_(&client), transaction_(nullptr) {}\n")
	out.WriteString("    explicit Queries(" + transactionType + "& transaction) noexcept : client_(nullptr), transaction_(&transaction) {}\n\n")
	for _, query := range in.Queries {
		returnType := "void"
...
func renderUserverMethod(out *strings.Builder, query model.AnalyzedQuery, options Options) {
	...
	call := "this->transaction_ != nullptr\n        ? this->transaction_->Execute(sqlc_query" + args + ")\n        : this->client_->ExecuteQuery(sqlc_query" + args + ")"
	if query.Command == model.Exec {
