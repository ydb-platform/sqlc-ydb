...
The generator writes one `queries.ts` file containing the implementation and its exported parameter and result type aliases. SQL appears directly in indented tagged templates such as `sql<[GetAuthorRow]>\`SELECT ...\``. Explicit `DECLARE` statements remain in the SQL, including their type spelling, comments and formatting. SQL source lines are indented within the method; string literals and non-parameter local bindings are preserved.

The pinned `@ydbjs/query` SDK prepends declarations in its public `Query.text` getter and offers no option to disable that behavior. For queries with explicit declarations, generated code first binds any inferred parameters, then captures `stmt.text` as an own read-only `text` property before binding the explicitly declared parameters. This prevents duplicate declarations while retaining SDK-generated declarations for inferred parameters. It uses the same SDK query object, configuration callback, retry handling and caller-owned transaction. Queries without explicit declarations use the SDK normally. The SDK compatibility test captures actual `executeQuery` requests to verify source declarations, typed empty and populated lists, retries and transactions.
