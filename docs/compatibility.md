...
`Bytes` is accepted as the canonical binary `String` type and `Text` as the canonical Unicode `Utf8` type in schema, declaration and nested type positions. Struct field identity is name-based rather than declaration-order-based.
