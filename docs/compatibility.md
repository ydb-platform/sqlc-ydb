...
- `--no-remote` skips the optional release check for `version`; analysis and generation always run locally. It conflicts with `version --upgrade`. `--remote` and upstream's `--no-database` are unsupported. `init` creates a version 2 configuration (`--v2` is also accepted); `version --verbose` and `version --upgrade` are sqlc-ydb extensions.
...
`DECLARE $books AS List<Struct<...>>` can supply an `AS_TABLE($books)` source. The analyzer resolves Struct fields by name regardless of declaration order and checks the explicit SELECT projection against the explicit INSERT/UPSERT target columns. INSERT/UPSERT SELECT currently accepts one SELECT input; wildcard and UNION inputs remain unsupported because their server column order is not established by this offline contract. UPDATE/DELETE ON SELECT can use qualified or unqualified wildcards because those statements match by result name. All nine language generators support list-of-struct parameters with fields supported by the selected runtime's scalar and optional-scalar mapping. Nested containers and nested structs remain unsupported by these bindings. Generated APIs expose named item types and preserve the declared type for empty lists. See the [batch example](../examples/batch/README.md).
