...
`runtime: jooq` translates named YQL queries into the jOOQ DSL. SQL files remain its input; generated methods contain no embedded SQL statements. Unsupported constructs fail generation with the query name and offending syntax, without a plain-SQL fallback. The prototype covers all 40 queries in the five example families: SELECT, projections, aliases, LEFT JOIN, comparisons and logical conditions, ordering, LIMIT, GROUP BY with COUNT(*), INSERT/UPSERT, UPDATE, DELETE and RETURNING. Its YQL function subset includes SetIsDisjoint, ToSet and Yson::ConvertToStringList; other functions require an explicit implementation.
