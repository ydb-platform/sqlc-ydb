...
The typed DSL supports noncorrelated scalar `IN (SELECT ...)` and `NOT IN (SELECT ...)` predicates with independent inner aliases and table mappings. Tuple-key subqueries require explicit `DECLARE` statements to use the existing typed JDBC path, or `runtime: jdbc`/`ydb`. The pinned jOOQ row projection renders `SELECT row (...)`, which does not preserve YQL's required single-tuple-column form `SELECT (...)`; the DSL rejects this form instead of changing the query.
...
Multi-statement scripts with scalar parameters use this declared-query path and execute as one request with table mapping applied to every statement. They support `:exec` without results and `:one`/`:many` with one SELECT or DML RETURNING result. At least one parameter must have an explicit DECLARE; the named preparation path adds declarations for other inferred parameters. Without an explicit declaration, including scripts without parameters, use `runtime: jdbc` or `ydb`. Scripts with List<Struct> parameters retain the [structured batch execution](#structured-batch-execution) restrictions.
