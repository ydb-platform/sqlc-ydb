...
The [authors example](../examples/authors/kotlin) builds all three profiles from the same [SQL](../examples/authors/queries.sql). The `batch`, `booktest`, `jets`, and `ondeck` examples generate native Query SDK APIs. Run `make generate` to update the checked-in examples.
