...
async fn jets_smoke(client: &mut ydb::QueryClient) -> ydb::YdbResult<()> {
    exec(client, include_str!("../../../../examples/jets/schema.sql")).await?;
    exec(
        client,
...
async fn ondeck_smoke(client: &mut ydb::QueryClient) -> ydb::YdbResult<()> {
    for migration in [
        include_str!("../../../../examples/ondeck/schema/0001_city.sql"),
        include_str!("../../../../examples/ondeck/schema/0002_venue.sql"),
