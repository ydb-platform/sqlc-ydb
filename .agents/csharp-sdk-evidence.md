...
## Mixed scripts with one result set

Ydb.Sdk 0.35.0 [`YdbDataReader.CloseAsync`](https://github.com/ydb-platform/ydb-dotnet-sdk/blob/9cd82d95f41804e3938a76f15b203fcc16a7562b/src/Ydb.Sdk/src/Ado/YdbDataReader.cs#L835-L864) reads at most one additional row before deciding whether the stream was consumed; closing an unread stream invalidates the connection and breaks an attached transaction. Generated ADO.NET `:one` methods for multi-statement scripts therefore save the first row and consume `ReadAsync` to completion before returning it. The shared analyzer guarantees one result set. `NextExecPart` propagates failed stream status, and the wrapper does not commit, roll back, or replace a caller-owned transaction. Single-statement output is unchanged.

Dapper 2.1.79 [`QueryRowAsync`](https://github.com/DapperLib/Dapper/blob/2.1.79/Dapper/SqlMapper.Async.cs#L477-L508), used by `QueryFirstAsync`, already consumes remaining rows and result sets; buffered `QueryAsync` also consumes them. The Dapper generator needs no additional drain. `TestDMLScriptsCsharpBuildsAgainstPublishedSDK` compiles both generated profiles against the pinned packages, and the `csharp` subtest of `TestLiveYDBDMLScripts` verifies first-row retention, transaction reuse and rollback, empty results, late DML errors, and connection reuse on the real server.
