...
| Generate example outputs before verification | Example sources and handwritten harnesses remain tracked; generated files can be absent on checkout. SDK review PRs deliberately track only generated outputs, and merged outputs stay tracked; ignore rules cover outputs not yet accepted. Make targets and CI generate before runtime builds; checks reject drift in any tracked example outputs. See [development](development.md#generated-runtime-checks). |
