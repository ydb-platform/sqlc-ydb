...
| Preserve explicit declarations and SQL formatting | Retain source `DECLARE` statements, indentation and line breaks in generated SQL. The analyzer records declared parameter names once; adapters avoid duplicate SDK declarations without deleting source text. See [architecture](architecture.md). |
...
| SQL at its execution site | Emit SQL directly in query/execute/prepare calls, with an outer indentation level matching that call and unchanged relative SQL indentation. Use aligned string concatenation when multiline literals cannot preserve both layout and runtime text. See [generated code layout](development.md#generated-code-layout). |
