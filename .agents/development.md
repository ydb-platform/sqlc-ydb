...
`TestLiveYDBDMLScripts` executes complete result-free scripts through both generated Go profiles, checking shared parameters, INSERT/UPSERT/UPDATE/DELETE order, read-your-writes, caller transaction commit/rollback, later-statement failure, cancellation and reuse. It counts generated Exec calls to reject client-side statement splitting. Run `YDB_CONNECTION_STRING=grpc://localhost:2136/local go test -p 1 -count=1 -timeout=240s ./internal/endtoend -run '^TestLiveYDBDMLScripts$' -v` sequentially with other live suites; set `SQLC_YDB_TEST_MAVEN=mvn` for jOOQ script execution with mapped tables. Stable and nightly CI run the suite in a separate step; stable CI enables jOOQ and uploads `coverage-dml-scripts-live.out`. `TestDMLScriptsGeneratedGoCompiles` compiles the generated Go clients and execution harness without a database.
