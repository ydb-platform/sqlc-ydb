...
| SDK compatibility | **Automated pinned-SDK gate complete; maintainer review incomplete** | The current main CI run compiled the published C#, Java and Rust SDK targets, all Dapper examples, and the TypeScript, Rust and PHP examples. It ran sequential YDB acceptance for Go, Python, C#, Java, Dapper, TypeScript, Rust, PHP and C++ native/userver. The Go examples pin `v3.151.1`, which was also the latest stable ydb-go-sdk release when checked. Exact pins and inspected API sources are in [SDK evidence](sdk-evidence.md). Nine language-specific generated-output reviews were prepared. Go PR #3 is merged; PRs #2 and #4 through #10 remain open for the corresponding SDK maintainers. Maintainer review and profile readiness therefore remain incomplete. |
