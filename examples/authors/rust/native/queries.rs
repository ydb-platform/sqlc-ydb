...
impl<'a> Queries<'a> {
    ...
    pub async fn get_author_name(&mut self, author_id: u64) -> ydb::YdbResult<GetAuthorNameRow> {
        let call = self.client.query_result_set(GET_AUTHOR_NAME)
            .param("$author_id", author_id);
        let result_set = call.await?;
        let mut row = result_set.rows().next().ok_or(ydb::YdbError::NoRows)?;
        Ok(GetAuthorNameRow {
    ...
    pub async fn create_author(&mut self, author_id: u64, author_name: String, biography: Option<String>) -> ydb::YdbResult<CreateAuthorRow> {
    ...
    pub async fn upsert_author(&mut self, author_id: u64, author_name: String, biography: Option<String>) -> ydb::YdbResult<()> {
        let call = self.client.exec(UPSERT_AUTHOR)
            .param("$author_id", author_id)
            .param("$author_name", author_name)
            .param("$biography", biography);
        call.await
    ...
    pub async fn delete_author(&mut self, author_id: u64) -> ydb::YdbResult<()> {
        let call = self.client.exec(DELETE_AUTHOR)
            .param("$author_id", author_id);
        call.await
    }
