...
impl<'a> Queries<'a> {
    ...
    pub async fn create_author(&mut self, author_id: u64, name: String, biography: Option<String>) -> ydb::YdbResult<CreateAuthorRow> {
        let call = self.client.query_result_set(CREATE_AUTHOR)
            .param("$author_id", author_id)
            .param("$name", name)
    ...
    pub async fn create_book(&mut self, book_id: u64, author_id: u64, isbn: String, book_type: String, title: String, year: i32, available: std::time::SystemTime, tags: String) -> ydb::YdbResult<CreateBookRow> {
        let call = self.client.query_result_set(CREATE_BOOK)
            .param("$book_id", book_id)
            .param("$author_id", author_id)
            .param("$isbn", isbn)
    ...
    pub async fn update_book(&mut self, title: String, tags: String, book_id: u64) -> ydb::YdbResult<()> {
        let call = self.client.exec(UPDATE_BOOK)
            .param("$title", title)
            .param("$tags", JsonParam(tags))
            .param("$book_id", book_id);
        call.await
    ...
    pub async fn get_biography(&mut self, author_id: u64) -> ydb::YdbResult<GetBiographyRow> {
        let call = self.client.query_result_set(GET_BIOGRAPHY)
            .param("$author_id", author_id);
        let result_set = call.await?;
        let mut row = result_set.rows().next().ok_or(ydb::YdbError::NoRows)?;
        Ok(GetBiographyRow {
