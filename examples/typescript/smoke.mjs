...
async function runAuthors() {
  await executeFile("../authors/schema.sql");
  try {
...
async function runBatch() {
  await executeFile("../batch/schema.sql");
  try {
    ...
    const available = 1788957296789123n;
    ...
    assert.equal(book.available, available);
    assert.equal(book.tags, '["history","science-fiction"]');
    ...
    assert.equal((await queries.getBiography(authorId)).biography, '{"born":1947}');
...
async function runBooktest() {
  ...
  try {
    ...
    assert.equal((await queries.getBook(bookId)).available, available);
...
async function runOndeck() {
  for (const migration of ["0001_city.sql", "0002_venue.sql", "0003_rename_venue.sql", "0004_add_created_at.sql", "0005_drop_column.sql"]) {
    await executeFile(`../ondeck/schema/${migration}`);
  }
  try {
    ...
    const createdAt = 1788948672345123n;
    ...
    assert.equal((await queries.getVenue({ slug: "roundhouse", city: "london" })).createdAt, createdAt);
