// Code generated by sqlc-ydb. DO NOT EDIT.
// source: venue.sql
package ondeck

import (
	"context"
	"database/sql"
	"github.com/ydb-platform/ydb-go-sdk/v3/table"
	"github.com/ydb-platform/ydb-go-sdk/v3/types"
)

const queryListVenues = `-- name: ListVenues :many
DECLARE $city AS Utf8;
SELECT id, slug, name, city, status, statuses, spotify_playlist, songkick_id, tags, created_at
FROM venue
WHERE city = $city
ORDER BY name;`

func (q *Queries) ListVenues(ctx context.Context, arg string) ([]ListVenuesRow, error) {
	rows, err := q.db.QueryContext(ctx, queryListVenues, sql.Named("city", arg))
	if err != nil {
		return []ListVenuesRow(nil), err
	}
	defer rows.Close()
	items := []ListVenuesRow(nil)
	for rows.Next() {
		var row ListVenuesRow
		if err := rows.Scan(&row.ID, &row.Slug, &row.Name, &row.City, &row.Status, &row.Statuses, &row.SpotifyPlaylist, &row.SongkickID, &row.Tags, &row.CreatedAt); err != nil {
			return nil, err
		}
		items = append(items, row)
	}
	return items, rows.Err()
}

const queryDeleteVenue = `-- name: DeleteVenue :exec
DECLARE $slug AS Utf8;
DELETE FROM venue
WHERE slug = $slug AND slug = $slug;`

func (q *Queries) DeleteVenue(ctx context.Context, arg string) error {
	_, err := q.db.ExecContext(ctx, queryDeleteVenue, sql.Named("slug", arg))
	return err
}

const queryGetVenue = `-- name: GetVenue :one
DECLARE $slug AS Utf8;
DECLARE $city AS Utf8;
SELECT id, slug, name, city, status, statuses, spotify_playlist, songkick_id, tags, created_at
FROM venue
WHERE slug = $slug AND city = $city;`

func (q *Queries) GetVenue(ctx context.Context, arg GetVenueParams) (GetVenueRow, error) {
	var row GetVenueRow
	err := q.db.QueryRowContext(ctx, queryGetVenue, sql.Named("slug", arg.Slug), sql.Named("city", arg.City)).Scan(&row.ID, &row.Slug, &row.Name, &row.City, &row.Status, &row.Statuses, &row.SpotifyPlaylist, &row.SongkickID, &row.Tags, &row.CreatedAt)
	return row, err
}

const queryCreateVenue = `-- name: CreateVenue :one
DECLARE $id AS Uint64;
DECLARE $slug AS Utf8;
DECLARE $name AS Utf8;
DECLARE $city AS Utf8;
DECLARE $created_at AS Optional<Timestamp>;
DECLARE $spotify_playlist AS Utf8;
DECLARE $status AS Utf8;
DECLARE $statuses AS Optional<Json>;
DECLARE $tags AS Optional<Json>;
INSERT INTO venue (
    id,
    slug,
    name,
    city,
    created_at,
    spotify_playlist,
    status,
    statuses,
    tags
) VALUES (
    $id,
    $slug,
    $name,
    $city,
    $created_at,
    $spotify_playlist,
    $status,
    $statuses,
    $tags
) RETURNING id;`

func (q *Queries) CreateVenue(ctx context.Context, arg CreateVenueParams) (CreateVenueRow, error) {
	var row CreateVenueRow
	err := q.db.QueryRowContext(ctx, queryCreateVenue, sql.Named("id", arg.ID), sql.Named("slug", arg.Slug), sql.Named("name", arg.Name), sql.Named("city", arg.City), sql.Named("created_at", arg.CreatedAt), sql.Named("spotify_playlist", arg.SpotifyPlaylist), sql.Named("status", arg.Status), table.ValueParam("$statuses", types.NullableJSONValue(arg.Statuses)), table.ValueParam("$tags", types.NullableJSONValue(arg.Tags))).Scan(&row.ID)
	return row, err
}

const queryUpdateVenueName = `-- name: UpdateVenueName :one
DECLARE $name AS Utf8;
DECLARE $slug AS Utf8;
UPDATE venue
SET name = $name
WHERE slug = $slug
RETURNING id;`

func (q *Queries) UpdateVenueName(ctx context.Context, arg UpdateVenueNameParams) (UpdateVenueNameRow, error) {
	var row UpdateVenueNameRow
	err := q.db.QueryRowContext(ctx, queryUpdateVenueName, sql.Named("name", arg.Name), sql.Named("slug", arg.Slug)).Scan(&row.ID)
	return row, err
}

const queryVenueCountByCity = `-- name: VenueCountByCity :many
SELECT
    city,
    COUNT(*) AS venue_count
FROM venue
GROUP BY city
ORDER BY city;`

func (q *Queries) VenueCountByCity(ctx context.Context) ([]VenueCountByCityRow, error) {
	rows, err := q.db.QueryContext(ctx, queryVenueCountByCity)
	if err != nil {
		return []VenueCountByCityRow(nil), err
	}
	defer rows.Close()
	items := []VenueCountByCityRow(nil)
	for rows.Next() {
		var row VenueCountByCityRow
		if err := rows.Scan(&row.City, &row.VenueCount); err != nil {
			return nil, err
		}
		items = append(items, row)
	}
	return items, rows.Err()
}
