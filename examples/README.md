...
The `records`, `counters`, `streaming` and `namespaces` recipes are native sqlc-ydb examples. Their default configurations are offline and included in the same generation/Go checks; the counters README documents the optional connected configurations.
