...
import { Bytes, Int32, Json, Timestamp, Uint64, Utf8, Utf8Type } from "@ydbjs/value/primitive";
...
export type FindAuthorsParams = {
  readonly minAuthorId: bigint;
  readonly filterName: string | null;
};

export type FindAuthorsRow = {
  readonly author_id: bigint;
  readonly name: string;
};
...
export class Queries {
  ...
  async findAuthors(args: FindAuthorsParams, configure?: ConfigureQuery): Promise<FindAuthorsRow[]> {
    const stmt = this.#sql<[FindAuthorsRow]>(
      "SELECT author_id, name\n" +
      "FROM authors\n" +
      ...
      .parameter("min_author_id", new Uint64(args.minAuthorId))
      .parameter("filter_name", new Optional(args.filterName === null ? null : new Utf8(args.filterName), new Utf8Type()));
    configure?.(stmt);
    const [rows] = await stmt;
