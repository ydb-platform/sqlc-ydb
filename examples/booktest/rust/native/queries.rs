...
impl<'a> Queries<'a> {
    ...
    pub async fn get_book(&mut self, book_id: u64) -> ydb::YdbResult<GetBookRow> {
        let call = self.client.query_result_set(GET_BOOK)
    ...
    pub async fn books_by_title_year(&mut self, title: String, publication_year: i32) -> ydb::YdbResult<Vec<BooksByTitleYearRow>> {
        let call = self.client.query_result_set(BOOKS_BY_TITLE_YEAR)
            .param("$title", title)
    ...
    pub async fn books_by_tags(&mut self, tags: String) -> ydb::YdbResult<Vec<BooksByTagsRow>> {
        let call = self.client.query_result_set(BOOKS_BY_TAGS)
    ...
    pub async fn create_author(&mut self, author_id: u64, name: String) -> ydb::YdbResult<CreateAuthorRow> {
    ...
    pub async fn create_book(&mut self, book_id: u64, author_id: u64, isbn: String, book_type: String, title: String, publication_year: i32, available: std::time::SystemTime, tags: String) -> ydb::YdbResult<CreateBookRow> {
    ...
    pub async fn update_book(&mut self, book_id: u64, title: String, tags: String) -> ydb::YdbResult<()> {
    ...
    pub async fn update_book_isbn(&mut self, book_id: u64, title: String, tags: String, isbn: String) -> ydb::YdbResult<()> {
        let call = self.client.exec(UPDATE_BOOK_ISBN)
            .param("$book_id", book_id)
            .param("$title", title)
            .param("$tags", JsonParam(tags))
            .param("$isbn", isbn);
    ...
    pub async fn delete_author_before_year(&mut self, author_id: u64, publication_year: i32) -> ydb::YdbResult<()> {
        let call = self.client.exec(DELETE_AUTHOR_BEFORE_YEAR)
            .param("$author_id", author_id)
            .param("$publication_year", publication_year);
        call.await
    }
    ...
    pub async fn say_hello(&mut self, name: String) -> ydb::YdbResult<SayHelloRow> {
        let call = self.client.query_result_set(SAY_HELLO)
            .param("$name", name);
        let result_set = call.await?;
        let mut row = result_set.rows().next().ok_or(ydb::YdbError::NoRows)?;
        Ok(SayHelloRow {
