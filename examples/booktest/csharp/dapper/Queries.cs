...
public sealed class Queries
{
    ...
    {
        var command = new CommandDefinition(SqlUpdateBook, new YdbParameters(new YdbParameter("$book_id", DbType.UInt64, args.BookID), new YdbParameter("$title", DbType.String, args.Title), new YdbParameter("$tags", YdbValue.MakeJson(args.Tags))), _transaction, cancellationToken: cancellationToken);
        await _connection.ExecuteAsync(command).ConfigureAwait(false);
    }
    ...
    {
        var command = new CommandDefinition(SqlDeleteAuthorBeforeYear, new YdbParameters(new YdbParameter("$author_id", DbType.UInt64, args.AuthorID), new YdbParameter("$publication_year", DbType.Int32, args.PublicationYear)), _transaction, cancellationToken: cancellationToken);
        await _connection.ExecuteAsync(command).ConfigureAwait(false);
    }
